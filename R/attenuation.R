...
attenuation <- function(frequency, temp = 20, rh = 60, pa = 101325, dist0, dist, hab.att.coef = 0.02) {
  # check arguments
  ...
  y <- (temp / To)**(-5 / 2) * (0.01275 * exp(-2239.1 / temp) * 1 / (fr0 + frequency**2 / fr0) + z)


  atm_att_coef <- 8.686 * frequency**2 * ((1.84e-11 * 1 / (pa / pr) * sqrt(temp / To)) + y)
  ...
  outdf <- data.frame(frequency = frequency, dist = dist, geometric.attenuation = geom_att, atmopheric.attenuation = atm_att, habitat.attenuation = hab_att, combined.attenuation = total_att)
