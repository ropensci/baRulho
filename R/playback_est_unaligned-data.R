...
#' 
#' The data contains a subset of the selections in the example data 'playback_est' but in this subset the re-recorded sounds are not aligned in time with the corresponding reference sounds (see \code{\link{realign_test_sounds}} for more details on aligning sounds). This data set is intended mostly for using as an example in \code{\link{realign_test_sounds}}. The data contains recordings of \emph{Phaethornis longirostris} (Long-billed Hermit) songs from different song types (column 'sound.id') that were broadcast and re-recorded at 4 distances (1m, 5m, 10m, 15m, column 'distance'). The data was created by the function \code{\link[warbleR]{selection_table}} from the \code{\link{warbleR}} package.
