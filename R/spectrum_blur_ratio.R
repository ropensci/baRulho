...
#' pb = getOption("pb", TRUE), method = getOption("method", 1), ssmooth = 50,
#' output = NULL, spectra = FALSE, img = FALSE, res = 150,
#' hop.size = getOption("hop.size", 11.6), wl = getOption("wl", NULL),
#'  ovlp = getOption("ovlp", 70), palette = viridis::viridis,
#' collevels = seq(-120, 0, 5), dest.path = NULL,
...
#' @return Object 'X' with two additional columns, 'reference' and 'spectrum.blur.ratio', containing the id of the sound used as reference and the computed spectrum blur ratio values, respectively. If \code{img = TRUE} it also returns 1 image file (in 'jpeg' format) for each comparison showing spectrograms of both sounds and the overlaid power spectrum (as probability mass functions (PMF)). Spectrograms are shown within the frequency range of the reference sound, showing vertical lines with the start and end of sounds to allow users to visually check alignment. If \code{spectra = TRUE} the output would include power spectra for all sounds as attributes ('attributes(X)$spectra').
#' @export
...
#'
#'   # using method 1
#'   spectrum_blur_ratio(X = rerecorded_est)
#'
#'   # using method 2
#'   # spectrum_blur_ratio(X = rerecorded_est, method = 2)
#'
#'   # get power spectra
#'   sbr <- spectrum_blur_ratio(X = rerecorded_est, spectra = TRUE)
...
spectrum_blur_ratio <-
  function(X,
    ...
    blur_sp_FUN <-
      function(x, res, ovlp, wl, collevels, palette, ...) {
        ...
        if (sgnl == rfrnc |
          ...
          rfrnc.spc <-
            specs.list[[which(names(specs.list) == rfrnc)]]
