test_that("using extended table and both marker", {
  ...
  # save master file
  writeWave(object = attr(test_sounds_est, "wave.objects")[[1]], file.path(td, "master.wav"))

  options(sound.files.path = td, pb = T)
  
  pks <- find_markers(X = master_est, test.files = unique(test_sounds_est$sound.files)[2])
