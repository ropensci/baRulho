...
This vignette shows how to create synthetic sounds for playback experiments and how to align re-recorded sounds to their reference once playback has been coducted. For this vignette we will need a few more packages:
...
Rraven::exp_est(X = synth.l, single.file = TRUE, path = td, file.name = "synth_sounds.wav")

```

```{r, eval = TRUE, echo = FALSE, fig.height=5, dpi=120}
...
# plot spectro
seewave::spectro(readWave(file.path(td, "synth_sounds.wav")),
...
synth_master_annotations <- master_sound_file(
  X = synth.l, file.name = "synthetic_master",
...
We will use example files from a degradation experiment that are included in the package as example data. The complete data set of the experiment is hosted in [this repository](https://figshare.com/articles/media/baRulho_an_R_package_to_quantify_animal_acoustic_signal_transmission_and_degradation/21559074):
...
Now we will save those files in a temporary working directory (those objects are extended seletion tables in the R environment) to resemble the usual case in which the acoustic data is found in sound files.
