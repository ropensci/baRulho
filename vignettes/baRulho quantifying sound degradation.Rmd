...
spectrograms(master.st, path = td, by.song = "sound.files", 
...
xc <- xcorr(master.st, compare.matrix = comp_mat, wl = 300, ovlp = 30, path = td, output = "list")
...
In this example data their are 4 recordings at increasing distances: 1m, 5m, 10m and 15m:
...
The output data frame is similar to input data, but includes two new columns ('reference' and 'blur.ratio') with the reference signal and blur ratio values. Note that `NAs` are returned for signals used as reference and 'ambient' noise selections.
...
The image show the spectrogram for the reference and re-recorded signal, as well as the envelopes of both signals overlaid in a single graph. Color are use to indicate to which signal spectrograms and envelopes belong to. The blur ratio value is also inlcuded.

The function can also return the amplitude spectrum contours when the argument `output = "list"`. This contours can be direclty input into a ggplot for looking at amplitude envelopes, their variation with distance and across signal types (and ambient noise if included):
...
As in `blur_ratio()`, `spectral_blur_ratio()` can also return the amplitude spectrum contours when the argument `output = "list"`:
...
Amplitude envelope correlation measures the similarity of two signals in the time domain. The `envelope_correlation()` function measures the envelope correlation coefficients between reference playback and re-recorded signals. Values close to 1 means very similar amplitude envelopes (i.e. little degradation has occurred). If envelopes have different lengths (which means signals have different lengths) cross-correlation is used and the maximum correlation coefficient is returned. Cross-correlation is achieved by sliding the shortest signal along the largest one and calculating the correlation at each step. As in the functions detailed above, 'signal.type' column must be used to indicate the function to only compare signals belonging to the same category.
...
Spectrum correlation measures the similarity of two signals in the frequency domain. Is similar to `spectral_correlation()` but no cross-correlation is applied as both signals are compared within the frequency range of the reference signal (so both spectra have the same length). Again, values range from 1 (identical frequency spectrum, i.e. no degradation) to 0. 
...
### Spectrographic distortion

Finallty, the function `spcc_distortion()` measures spectrographic cross-correlation as a metric of signal distortion of signals. Values close to 1 means very similar spectrograms (i.e. little signal distortion). The function is a wrapper on [warbleR](https://cran.r-project.org/package=warbleR)'s `xcorr()`. It can be run as follows:

```{r, eval = TRUE}

# run spcc distortion
spd <- spcc_distortion(X = playback_est, method = 1, pb = FALSE)
