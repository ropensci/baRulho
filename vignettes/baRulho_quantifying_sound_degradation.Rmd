...
To find the location of the start marker on these (simulated) re-recorded sound files we can use the [warbleR](https://cran.r-project.org/package=warbleR) functions `xcorr()` to run a cross-correlation of the marker across the sound files and `find_peaks()` to determine the exact time in which the marker are found. See an example of how this works below (check [warbleR](https://cran.r-project.org/package=warbleR) documentation for more information on how these functions work): 
...
Note that data was converted into an `extended_selection_table`, which is the data format that pretty much all [baRulho](https://marce10.github.io/baRulho/) functions take. 
...
Below are some examples from actual test signals that were re-recorded at 1, 30, 65, and 100 m respectively, and were later aligned using the start marker:
