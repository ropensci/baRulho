...
The combined information from these columns is used to identify the reference sounds for each test sound. The function `set_reference_sounds()` does exactly that. There are two possible experimental designs whe defining reference sounds (which is controlled by the argument 'method' in `set_reference_sounds()`):
...
<center><img src="plot_degradation_p1.jpeg" alt="degradation plot"></center>
...
sbr <- blur_ratio(test_sounds_est, dest.path = tempdir())
...
<center><img src="blur_ratio_example.gif" alt="time sync with realign_test_sounds"  width="500" height="400"></center>
...
The `env.smooth` argument could change envelope shapes and related measurements. The following code sets `env.smooth = 800`:
...
sa <- signal_to_noise_ratio(test_sounds_est, pb = FALSE, noise.ref = "custom")
...
# run noise profile
np <- noise_profile(X = test_sounds_est[test_sounds_est$distance > 5, ], mar = 0.05)
```
